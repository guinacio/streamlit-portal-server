...
async def serve_app(request: Request, response: Response, app_id: int):
    ...
    if auth_token:
        ...
        # SECURITY CHECK: Ensure the user has an active portal session
        portal_session_cookie = request.cookies.get("portal_session")
        
        # Also check for localhost-specific cookies or other variations
        if not portal_session_cookie:
            # Check all cookies that might contain portal session
            for cookie_name, cookie_value in all_cookies.items():
                if cookie_name == "portal_session":
                    portal_session_cookie = cookie_value
                    break
        ...
        if not portal_session_cookie:
            return HTMLResponse(
                content=create_access_denied_page_with_reason(
                    app_id,
                    "Access Denied", 
                    "No active portal session found. Please log in to the portal first."
                ),
                status_code=403
            )
        
        # Validate that the portal session is still active
        user_info = db.validate_portal_session(portal_session_cookie)
        if not user_info:
            return HTMLResponse(
                content=create_access_denied_page_with_reason(
                    app_id,
                    "Access Denied", 
                    "Portal session expired. Please log in to the portal again."
                ),
                status_code=403
            )
        ...
        # ADDITIONAL SECURITY CHECK: Ensure the token was generated for the same user as the portal session
        if user_id != user_info['id']:
            return HTMLResponse(
                content=create_access_denied_page_with_reason(
                    app_id,
                    "Access Denied", 
                    "Token-user mismatch. Please log in again."
                ),
                status_code=403
            )
        ...
        # Redirect to clean URL without the auth_token
        ...
        # Try per-app session cookie first (normal flow)
        session_data = validate_session_cookie(request, app_id)
        
        if not session_data:
            # Fallback: authenticate via portal session cookie
            portal_session_token = request.cookies.get("portal_session")
            ...
            if not portal_session_token:
                ...
                for cookie_name, cookie_value in all_cookies.items():
                    if cookie_name == "portal_session":
                        portal_session_token = cookie_value
                        break
            ...
            if portal_session_token:
                ...
                if user_info:
                    ...
                    # Verify user can access this app
                    accessible_apps = db.get_accessible_apps(user_info['id'])
                    has_access = any(app['id'] == app_id for app in accessible_apps)
                    
                    if has_access:
                        
                        # Create new per-app session
                        session_id = create_secure_session(user_info['id'], app_id)
        ...
        # Per-app session is valid - serve the app
        app_info = db.get_app_by_id(app_id)
        if not app_info:
            raise HTTPException(status_code=404, detail="App not found")
